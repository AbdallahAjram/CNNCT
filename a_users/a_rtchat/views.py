...
def chat_index(request):
    ...
    def ts_or_min(ts):
        return ts or datetime(1970, 1, 1, tzinfo=dt_timezone.utc)
    # Build a single combined sidebar list to match template expectations
    group_chats = list(group_chats)
    ...
    if query:
        ...
        try:
            ...
            qs = User.objects.filter(
                ...
                Q(profile__phone_number=digits)
            )[:10]
        except Exception:
            pass
        results = list(qs)
...
def chat_start_new(request):
    ...
    try:
        ...
        user_qs = User.objects.filter(Q(username__icontains=raw) | Q(profile__display_name__icontains=raw))
